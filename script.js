...
/* ===== MODAL HOUSE 3D TILT ===== */
const modalHouseAnim = document.getElementById('modal-house-anim');
if (modalHouseAnim) {
  modalHouseAnim.addEventListener('mousemove', (e) => {
    const rect = modalHouseAnim.getBoundingClientRect();
    const x = (e.clientX - rect.left) / rect.width  - 0.5;
    const y = (e.clientY - rect.top)  / rect.height - 0.5;
    const houseImg = modalHouseAnim.querySelector('.modal-house-img');
    if (houseImg) {
      houseImg.style.transform = `scale(1.08) rotateY(${x * 20}deg) rotateX(${-y * 20}deg)`;
    }
  });

  modalHouseAnim.addEventListener('mouseleave', () => {
    const houseImg = modalHouseAnim.querySelector('.modal-house-img');
    if (houseImg) {
...
const overlay = document.getElementById('plan-detail-overlay');
if (overlay) {
  overlay.addEventListener('click', closePlanDetail);
}
...
if (modalCta) {
  modalCta.addEventListener('click', closePlan);
}

// Modal navigation
const prevBtn = document.querySelector('.modal-nav.prev');
if (prevBtn) {
  prevBtn.addEventListener('click', () => changePlan('prev'));
}

const nextBtn = document.querySelector('.modal-nav.next');
if (nextBtn) {
  nextBtn.addEventListener('click', () => changePlan('next'));
}
